...
namespace WebMyWorldEC.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        private MyWorldECEntities1 db = new MyWorldECEntities1();
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            try
            {
                ...
                var user = JsonConvert.DeserializeObject<UserResponseOne>(responseString);
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            var userId = Convert.ToInt32(Request.Cookies["Id"].Value);
            var resultAuth = await ((Data.URL + "Users/ChangePassword").WithHeader("Authorization", Data.Token).PostJsonAsync(new { IdUser = userId, model.NewPassword, model.OldPassword })).ReceiveString();
            var passwordModel = JsonConvert.DeserializeObject<ErrorResponse>(resultAuth);

            if (passwordModel.Success)
            {
            ...
            {
                if(passwordModel.ErrorNum == 401)
                {
                    ModelState.AddModelError("ConfirmPassword", passwordModel.ErrorMessages);
                }
                else
                {
                    ModelState.AddModelError("OldPassword", passwordModel.ErrorMessages);
                }
